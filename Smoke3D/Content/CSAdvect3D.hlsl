...
Texture3D<min16float3>		g_txVelocity	: register (t2);
...
{
	// Velocity tracing
	const float3 vU = g_txVelocity[DTid];
	const float3 vTex = (DTid + 0.5) * g_vTexel - vU * g_fDeltaTime;

	// Update velocity and density
	g_rwPhiVel[DTid] = g_txPhiVel.SampleLevel(g_smpLinear, vTex, 0);
	g_rwPhiDen[DTid] = g_txPhiDen.SampleLevel(g_smpLinear, vTex, 0) * g_vDecay[g_fMacCormack];
}
